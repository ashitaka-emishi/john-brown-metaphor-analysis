...
DEFAULT_USER_AGENT = "JohnBrownMetaphorAnalysis/0.1 (scholarly research; contact=replace-with-email)"
...
def user_agent() -> str:
    agent = os.getenv("JOHN_BROWN_RESEARCH_USER_AGENT", DEFAULT_USER_AGENT)
    if "replace-with-email" in agent:
        raise LocChroniclingError("Configure JOHN_BROWN_RESEARCH_USER_AGENT with a real contact.")
    return agent
